    ...
    <div className={`${bgColor} rounded-lg p-3 ${onClick ? 'cursor-pointer hover:opacity-80 transition-opacity' : ''}`} onClick={onClick}>
      <div className="flex items-center justify-between">
        <div className="flex flex-col">
          <span className="text-sm text-[#6F7281]">{title}</span>
          <span className="text-xs text-[#6F7281] opacity-75">{count} {countLabel}</span>
        </div>
        <div className="flex items-center gap-2">
          <span className="text-sm font-medium text-[#282B3A]">{amount}</span>
